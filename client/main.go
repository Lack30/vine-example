...
func call(i int, c client.Client) {
	...
	req := c.NewRequest("go.vine.srv.example", "Example.Call", &example.Request{
		Name: "John",
	})
...
func stream(i int, c client.Client) {
	...
	req := c.NewRequest("go.vine.srv.example", "Example.Stream", &example.StreamingRequest{})
...
func pingPong(i int, c client.Client) {
	// Create new request to service go.vine.srv.example, method Example.Call
	// Request can be empty as its actually ignored and merely used to call the handler
	req := c.NewRequest("go.vine.srv.example", "Example.PingPong", &example.StreamingRequest{})
